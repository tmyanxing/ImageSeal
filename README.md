# ImageSeal - 图片水印工具

> 一款简单易用的图片水印添加工具，支持自定义水印文字、颜色、透明度、角度等参数，生成斜向重复的透明水印效果。

[![Build and Release](https://github.com/YOUR_USERNAME/ImageSeal/actions/workflows/build-release.yml/badge.svg)](https://github.com/YOUR_USERNAME/ImageSeal/actions/workflows/build-release.yml)
[![Release](https://img.shields.io/github/v/release/YOUR_USERNAME/ImageSeal)](https://github.com/YOUR_USERNAME/ImageSeal/releases)
[![Downloads](https://img.shields.io/github/downloads/YOUR_USERNAME/ImageSeal/total)](https://github.com/YOUR_USERNAME/ImageSeal/releases)

## 📥 直接下载使用

**不想配置开发环境？直接下载运行！**

1. 进入 [Releases 页面](https://github.com/YOUR_USERNAME/ImageSeal/releases)
2. 下载最新版本的 `ImageSeal-vX.X.X-win-x64.zip`
3. 解压到任意目录
4. 双击运行 `ImageSeal.exe`
5. 打开浏览器访问 http://localhost:30032
6. 开始使用！

> **注意**：首次运行可能需要允许防火墙访问。

## 🎯 功能特点

- ✅ 上传图片并实时预览
- ✅ 自定义水印文字
- ✅ 可调节字体大小、颜色、透明度
- ✅ 可调节水印旋转角度（斜向效果）
- ✅ 可调节水印间距
- ✅ 生成透明重复水印
- ✅ 一键下载带水印的图片
- ✅ 支持打包成独立 EXE 程序

## 🖼️ 界面预览

应用界面分为两个部分：
- **左侧**：上传图片和水印设置
- **右侧**：水印效果预览和下载

## 🚀 快速开始

### 环境要求

- Node.js 18+
- .NET 8.0 SDK
- Windows 10/11（如需打包 EXE）

### 开发模式

1. **安装前端依赖**
   ```bash
   cd frontend/imageseal-ui
   npm install
   ```

2. **一键启动前后端**
   - 按 `Ctrl+Shift+P` 打开命令面板
   - 输入 `Tasks: Run Task`
   - 选择 `🚀 启动全栈应用 (前端+后端)`
   
   或者手动启动：
   ```bash
   # 终端1: 启动后端
   cd backend/ImageSeal.Api
   dotnet run

   # 终端2: 启动前端
   cd frontend/imageseal-ui
   npm run dev
   ```

3. **访问地址**
   - 前端开发服务器: http://localhost:3000
   - 后端 API: http://localhost:5000
   - Swagger 文档: http://localhost:5000/swagger

### 生产模式

1. **构建前端**
   ```bash
   cd frontend/imageseal-ui
   npm run build
   ```
   构建产物会自动输出到 `backend/ImageSeal.Api/wwwroot` 目录

2. **运行后端**
   ```bash
   cd backend/ImageSeal.Api
   dotnet run
   ```

3. **访问应用**
   打开浏览器访问 http://localhost:5000 即可使用完整应用

## 📦 打包成 EXE

### 方式一：使用 VSCode Task（推荐）

1. 按 `Ctrl+Shift+P` 打开命令面板
2. 输入 `Tasks: Run Task`
3. 选择 `🔨 打包成 EXE (Windows x64)`
4. 等待打包完成，EXE 文件位于 `publish` 目录

### 方式二：命令行打包

```bash
# 1. 先构建前端
cd frontend/imageseal-ui
npm run build

# 2. 打包后端为单文件 EXE
cd ../../backend/ImageSeal.Api
dotnet publish -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -o ../../publish
```

### 运行 EXE

1. 进入 `publish` 目录
2. 双击运行 `ImageSeal.exe`
3. 打开浏览器访问 http://localhost:5000

## 📁 项目结构

```
ImageSeal/
├── .vscode/                    # VSCode 配置
│   └── tasks.json             # 任务配置
├── backend/                    # 后端项目
│   └── ImageSeal.Api/         # ASP.NET Core Web API
│       ├── Controllers/       # API 控制器
│       │   └── WatermarkController.cs
│       ├── Models/            # 数据模型
│       │   └── WatermarkModels.cs
│       ├── Services/          # 业务服务
│       │   └── WatermarkService.cs
│       ├── Program.cs         # 入口程序
│       ├── appsettings.json   # 配置文件
│       └── wwwroot/           # 静态文件（前端构建输出）
├── frontend/                   # 前端项目
│   └── imageseal-ui/          # Vue 3 项目
│       ├── src/               # 源代码
│       │   ├── api/          # API 接口
│       │   ├── App.vue       # 根组件
│       │   └── main.js       # 入口文件
│       ├── public/           # 静态资源
│       ├── index.html        # HTML 模板
│       ├── package.json      # 依赖配置
│       └── vite.config.js    # Vite 配置
├── publish/                    # EXE 发布目录
└── README.md
```

## 🛠️ 技术栈

### 后端
- ASP.NET Core 8.0
- SixLabors.ImageSharp (图像处理)
- SixLabors.ImageSharp.Drawing (水印绘制)

### 前端
- Vue 3
- Vite 7
- Ant Design Vue 4
- Axios

## 📝 API 文档

启动后端后访问 http://localhost:5000/swagger 查看完整 API 文档

### 水印接口

#### 添加水印

```
POST /api/watermark/add
Content-Type: multipart/form-data

参数:
- file: 图片文件 (必填)
- watermarkText: 水印文字 (必填)
- fontSize: 字体大小 (默认: 30)
- color: 水印颜色 (默认: #888888)
- opacity: 透明度 0-100 (默认: 30)
- angle: 旋转角度 (默认: -30)
- spacing: 水印间距 (默认: 100)

返回:
{
  "success": true,
  "message": "水印添加成功",
  "data": {
    "imageBase64": "...",
    "mimeType": "image/png",
    "fileName": "image_watermarked.png"
  }
}
```

#### 健康检查

```
GET /api/watermark/health

返回:
{
  "success": true,
  "message": "服务正常运行",
  "data": "OK"
}
```

## 🎯 VSCode Tasks 说明

| 任务名称 | 说明 |
|---------|------|
| 🚀 启动全栈应用 (前端+后端) | 同时启动前后端开发服务器 |
| 启动前端开发服务器 | 仅启动前端 (端口 3000) |
| 启动后端API服务 | 仅启动后端 (端口 5000) |
| 📦 构建前端并发布到后端 | 构建前端并输出到 wwwroot |
| 🎯 运行后端（生产模式） | 以 Release 模式运行后端 |
| 🔨 打包成 EXE (Windows x64) | 打包成独立 EXE 程序 |
| 📥 安装前端依赖 | 安装前端 npm 依赖 |

## 🔧 常见问题

### Q: 如何发布新版本？
A: 参考 [RELEASE.md](RELEASE.md) 文档，使用 Git 标签触发自动构建。

### Q: 如何验证下载文件的完整性？
A: 
```powershell
# 计算下载文件的 SHA256 哈希
Get-FileHash ImageSeal-v1.0.0-win-x64.zip -Algorithm SHA256

# 与 .sha256 文件中的值对比
Get-Content ImageSeal-v1.0.0-win-x64.zip.sha256
```

### Q: EXE 文件很大怎么办？
A: 由于使用了 `--self-contained` 参数，EXE 包含了完整的 .NET 运行时，文件较大是正常的。如果目标机器已安装 .NET 8 运行时，可以去掉此参数减小体积。

### Q: 水印字体不正确？
A: 程序会优先使用微软雅黑字体，如果系统没有该字体，会回退到 Arial。

### Q: 支持哪些图片格式？
A: 支持 JPG、PNG、GIF、BMP、WebP 格式的图片。

## 🤝 贡献指南

欢迎提交 Issue 和 Pull Request！

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'feat: Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

## 📦 自动化发布

本项目配置了 GitHub Actions 自动化构建：

- **触发方式**：推送 `v*` 标签（如 `v1.0.0`）
- **构建产物**：Windows x64 单文件 EXE
- **发布位置**：GitHub Releases
- **详细文档**：查看 [RELEASE.md](RELEASE.md)

快速发布新版本：
```bash
git tag v1.0.0
git push origin v1.0.0
```

## 📄 许可证

MIT License

---

**ImageSeal** - 让水印添加变得简单！ 🎨
